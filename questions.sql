...
UPDATE members 
SET 
    member_address = 'Fatima Jinnah Women Uniersity Rawalpindi'
WHERE
    member_id = 'C101'
...
CREATE TABLE books_count AS SELECT b.isbn, b.book_title, COUNT(ist.issued_id) AS no_issued FROM
    books AS b
        JOIN
...
SELECT 
    b.category, SUM(b.rental_price) AS rental_price, COUNT(*)
FROM
    books AS b
        JOIN
    issued_status AS ist ON ist.issued_book_isbn = b.isbn
...
FROM
    employees AS e1
        JOIN
    branches AS b ON b.branch_id = e1.branch_id
join employees as e2

-- Task 11. Create a Table of Books with Rental Price Above a Certain Threshold

CREATE TABLE books_price_greater_than_seven AS SELECT * FROM
    books
WHERE
    rental_price > 7
